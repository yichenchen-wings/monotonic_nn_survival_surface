...
parser.add_argument(
    '--best_hyper_dir', 
    help='absolute path to where the best hyper param is stored, best hyperparam need to be in a file called best_hyper_params.json'
)
...
N_GPUS = args.num_gpu
TUNE_HYPER = True if args.tune == 'y' else False
TUNE_LR = True if args.tunelr == 'y' else False# only effective if TUNE_HYPER is False
TRANS_ONLY=False

DIR_BEST_HYPER = args.best_hyper_dir

N_TRIALS_TUNE = args.n_trials_tune
N_EPOCHS_TUNE = args.n_epochs_tune
N_EPOCHS_TRAIN = args.n_epochs_train
...
# %%
print(f'n entries in the tunings set: {len(ds_train_tune)}')

# %%
print(f'n entries in trainning set: {len(ds_train)}')

# %%
print(f'n entries in validation set: {len(ds_val)}')
...
if TUNE_HYPER:
    ...
    # Opening JSON file
    with open(os.path.join(DIR_BEST_HYPER,"best_hyper_params.json"), 'r') as openfile:

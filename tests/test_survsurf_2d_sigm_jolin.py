...
def test_SurvSurf2DSigm_shape(batch_size, hidden_dim, n_layers, feat_size, dropout, zero_at_t0):
...
def test_SurvSurf2DSigm_monotone(seed, hidden_dim, n_layers, dropout, zero_at_t0):
...
def test_SurvSurf2DSigm_range(seed, zero_at_t0):
    torch.manual_seed(seed)
    batch_size = 30
    feat_size = 2
...
def test_SurvSurf2DSigm_max(seed, zero_at_t0):
    torch.manual_seed(seed)
    batch_size = 30
    feat_size = 2
...
def test_SurvSurf2DSigm_min(seed, zero_at_t0):
    torch.manual_seed(seed)
    batch_size = 30
    feat_size = 2
    ts = torch.rand(batch_size, 1) * 1000
